...
def test_ber(fs=48000):
    ...
    for sigma in np.r_[0.1:8.0:0.2]:
        Nbits = 100
        bits = np.int16(random.rand(Nbits,1)*15)
        ...
        QAM, M, t, M_prefix, t_prefix = mod_QAM16(bits, prefix, f0=1800, tbw=4,
                fs=48000, baud=300, shaped=True, plot=False)
        sig_n = QAM+ sigma*random.randn(len(QAM))

        # demodulate and decode bits with non-coherent and FM demodulators
        r,i = demod_QAM16(sig_n, t, f0=1800, fs=48000)
        corr_index = detect_sync(r, i, M_prefix, len(prefix), plot=False)
        symbols = decode_symbols(r, i, corr_index, M.real.ravel(),
                M.imag.ravel(), Nbits-len(rbits), plot=False)

        decoded = symbols_to_hex(symbols)
        errors = np.sum([s[0]!=s[1][0] for s in zip(decoded,
            bits.tolist()[6:])])

        BER.append(1.0*errors/len(bits.tolist()))

    # plot
    f = plt.figure()
    plt.loglog(1/(np.r_[0.1:8.1:0.2]), BER)
    plt.title("Empirical BER for QAM demodulation")
    plt.xlabel("SNR")
    plt.ylabel("BER")
